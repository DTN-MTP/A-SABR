...
impl<'id> RoutingOutput<'id> {
    pub fn lazy_get_for_unicast(&self, dest: NodeID) -> Option<FirstHop<'id>> {
...
pub fn dry_run_multicast<'id,'a>(
    bundle: &Bundle,
    at_time: Date,
    tree: PathFindingOutput<'id,'a>,
) -> Result<Vec<NodeID>, ASABRError> {
...
fn schedule_multicast<'id,'a,NM: NodeManager, CM: ContactManager>(
    bundle: &Bundle,
    curr_time: Date,
    tree: &PathFindingOutput<'id,'a>,
    ...
    // update_multicast(bundle, curr_time, targets, source_route.clone())
    todo!()
}
...
pub fn dry_run_unicast_tree<'id,'a,NM: NodeManager, CM: ContactManager>(
    bundle: &Bundle,
    at_time: Date,
    tree: &PathFindingOutput<'id,'a, >,
    with_exclusions: bool,
...
//         curr_route_borrowed.schedule(at_time, &bundle_to_consider)?;
...
//     Err(ASABRError::ScheduleError("Faulty dry run"))
// }
...
fn schedule_unicast<'id,'a,NM: NodeManager, CM: ContactManager>(
    bundle: &Bundle,
    curr_time: Date,
    tree: PathFindingOutput<'id,'a>,
    multigraph: &Multigraph<'id,NM,CM>,
