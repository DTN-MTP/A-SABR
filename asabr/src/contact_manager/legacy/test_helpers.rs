...
macro_rules! generate_common_tests {
    ($manager_fn:expr, $manager_type:ty) => {
        ...
        fn try_init_zero_duration_rejects_any_bundle() {
            ...
            assert!(
                manager.dry_run_tx(contact.into(), 5, &bp0(1.0)).is_none(),
        ...
        fn dry_run_volume_boundary() {
            ...
            assert!(
                manager.dry_run_tx(ti, C_START, &bp0(TOTAL_VOL + 1.0)).is_none(),
        ...
        fn dry_run_contact_timing_boundaries() {
            ...
            let ti = $crate::types::TimeInterval{start:C_START, end:C_END};
            assert!(
                manager.dry_run_tx(ti, C_END + 1, &bp0(1.0)).is_none(),
                "TEST FAILED: Expected None when at_time is past contact end."
            );
            assert!(
                manager.dry_run_tx(ti, C_END - 0, &bp0(600.0)).is_none(),
        ...
        fn tx_data_fields_are_correct() {
            let data = ($manager_fn)()
                .dry_run_tx($crate::types::TimeInterval{start:C_START, end:C_END}, C_START, &bp0(100.0))
                .unwrap();
            assert_eq!(
                data.expiration,
                C_END,
        ...
        fn single_prio_manager_ignores_priority_field() {
            ...
            let ti = $crate::types::TimeInterval{start:C_START, end:C_END};
            assert_eq!(
                manager.dry_run_tx(ti, C_START, &bp0(100.0)),
                manager.dry_run_tx(ti, C_START, &bp1(100.0)),
                "TEST FAILED: Single-priority manager should return identical timings for p0 and p1."
            );
            assert_eq!(
                manager.dry_run_tx(ti, C_START, &bp0(100.0)),
                manager.dry_run_tx(ti, C_START, &bp2(100.0)),
...
macro_rules! generate_auto_update_tests {
    ($manager_fn:expr, $p_manager_fn:expr) => {
        ...
        fn schedule_tx_saturation() {
            ...
            assert!(
                manager.schedule_tx(ti, C_START, &bp0(0.1)).is_none(),
        ...
        fn mid_prio_cascades_down_but_not_up() {
            ...
            assert!(
                manager.schedule_tx(ti, C_START, &bp1(5000.0)).is_some(),
...
macro_rules! generate_budget_tests {
    ($pb_manager_fn:expr) => {
        ...
        fn budget_hard_limits_per_priority() {
            ...
            assert!(
                manager
                    .dry_run_tx(ti, C_START, &bp0(BUDGET_P0 + 0.1))
                    .is_none(),
            ...
            assert!(
                manager
                    .dry_run_tx(ti, C_START, &bp1(BUDGET_P1 + 0.1))
                    .is_none(),
...
macro_rules! generate_budget_auto_update_tests {
    ($pb_manager_fn:expr) => {
        ...
        fn budget_priorities_are_independent() {
            ...
            assert!(
                manager.dry_run_tx(ti, C_START, &bp0(0.1)).is_none(),
                "TEST FAILED: Expected None -> p0 budget should be exhausted."
            );
            assert!(
                manager.dry_run_tx(ti, C_START, &bp2(100.0)).is_some(),
                "TEST FAILED: Expected Some -> p2 budget should be independent of p0."
            );
            assert!(
                manager.dry_run_tx(ti, C_START, &bp1(1.0)).is_some(),
