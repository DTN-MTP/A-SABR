...
macro_rules! define_node_manager {
    ($($bounds:tt)*) => {
        ...
        ///
        /// The `NodeRx` trait defines methods for dry-run (simulation) and actual scheduling of
        /// reception (rx) of a `Bundle` at specified times.
        /// This trait is useful for implementing custom logic for nodes that need to manage bundle
        /// data transfer in a time-dependent manner.
        ...
        /// A trait for managing and scheduling operations on nodes in a network.
        ///
        /// The `NodeTx` trait defines methods for dry-run (simulation) and actual scheduling of
        /// transmission (tx) of a `Bundle` at specified times.
        /// This trait is useful for implementing custom logic for nodes that need to manage bundle
        /// data transfer in a time-dependent manner.
        ...
        /// Implementation of `NodeRx` for boxed dynamic types (`Box<dyn NodeManager>`).
        impl NodeRx for Box<dyn NodeManager> {
            /// Delegates the dry_run method to the boxed object.
            fn dry_run_rx(&self, start: Date, end: Date, bundle: &Bundle) -> bool {
                (**self).dry_run_rx(start, end, bundle)
            }
        ...
        /// Implementation of `NodeTx` for boxed dynamic types (`Box<dyn NodeManager>`).
        impl NodeTx for Box<dyn NodeManager> {
            /// Delegates the dry_run method to the boxed object.
            fn dry_run_tx(&self, waiting_since: Date, start: Date, end: Date, bundle: &Bundle) -> bool {
                (**self).dry_run_tx(waiting_since, start, end, bundle)
            }
